...
def cartpole_dynamics(state, action, params):
    gravity, mass_cart, mass_pole, length_pole = params['gravity'], params['mass_cart'], params['mass_pole'], params['length_pole']
    ...
    numer_theta_dot_dot = -f_x * cos_theta - mass_pole * length_pole * theta_dot * theta_dot * cos_theta * sin_theta - (mass_cart + mass_pole) * gravity * sin_theta
...
st.header("Summary")
st.write("This app demonstrates optimal control of a cartpole system. The task is to apply a sequence of inputs to drive the system from the initial state to the target terminal state while minimizing an objective function.")

st.header("Optimal Control")
st.write("The optimal control problem is formulated using [CasADi](https://web.casadi.org/) and the direct multiple-shooting technique to transcribe it to a nonlinear program (NLP), which is solved using the [IPOPT](https://github.com/coin-or/Ipopt) solver. See https://web.casadi.org/blog/ocp/ for more details.")

st.header("Dynamics")
st.subheader("Equations of Motion")
st.write("The equations of motion for the cartpole system are derived using Lagrangian mechanics. The details are found at http://underactuated.mit.edu/acrobot.html.")
st.subheader("Numerical Integration of the Dynamics Ordinary Differential Equation")
st.write("The ordinary differential equation (ODE) governing the dynamics of the system are integrated numerically using the Runge-Kutta 4th-order method (RK4).")

st.header("Animation")
st.write("The cartpole is animated as a GIF using PyGame as a lightweight rendering engine. The animation rendering is an improved version of that found in the [Gymnasium cartpole environment](https://github.com/Farama-Foundation/Gymnasium/blob/main/gymnasium/envs/classic_control/cartpole.py).")
...
with st.form("options_form"):
    ...
    with option_cols_row1[2]:
        ...
        duration_end_hold_sec = st.slider("Animation Duration Pause at End (sec)", min_value=0, max_value=5, value=2)
    ...
    with option_cols_row2[0]:
        ...
        position_0 = st.slider("Position (m)", min_value=-position_threshold, max_value=position_threshold, value=0.0, step=0.1)
        ...
        angular_velocity_0_rps = st.slider("Angular Velocity (rev/s)", min_value=-2.0, max_value=2.0, value=0.0, step=0.1)
    ...
    with option_cols_row2[1]:
        ...
        position_min, position_max = st.slider("Position Constraint (m)", min_value=-2.0, max_value=2.0, value=(-2.0, 2.0), step=0.1)
        velocity_min, velocity_max = st.slider("Velocity Constraint (m/s)", min_value=-10.0, max_value=10.0, value=(-10.0, 10.0), step=0.5)
        angle_min_deg, angle_max_deg = st.slider("Angle Constraint (deg)", min_value=-360, max_value=360, value=(-360, 360), step=10)
        angular_velocity_min, angular_velocity_max = st.slider("Angular Velocity Constraint (rev/s)", min_value=-2.0, max_value=2.0, value=(-2.0, 2.0), step=0.1)

        angle_min, angle_max = angle_min_deg * (2 * np.pi / 360), angle_max_deg * (2 * np.pi / 360)
        angular_velocity_min, angular_velocity_max = angular_velocity_min * (2 * np.pi), angular_velocity_max * (2 * np.pi)

        force_min, force_max = st.slider("Force Constraint (N)", min_value=-50, max_value=50, value=(-20, 20), step=5)
...
def solve_optimal_control_problem(x0, xT, N, T, model_params, penalty_options, constraint_options):
    ...
    # ---- dynamic constraints --------
    f = lambda x, u: cartpole_dynamics(x, u, model_params) # dx/dt = f(x,u)
...
def animate(ani_state_action_time_series, target_state, fps, animation_options):
    with st.spinner("Creating frames..."):
        frames = [create_frame(time, state, action, target_state, animation_options) for time, state, action in ani_state_action_time_series]
    ...
    with st.spinner("Saving animation..."):
        ...
        frames[0].save(byte_stream, format="GIF", save_all=True, append_images=frames[1:], loop=0, duration=duration)
...
with plot_cols[0]:
    ...
    if show_animation:
        ...
        ani_state_action_time_series = [(t, s, a) for t, s, a in zip(ani_df.time, ani_df[state_fields].values, ani_df[action_fields].values)]
        target_state = xT
